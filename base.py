...
class Base:
    def __init__(self):
        ...
        self.bases=[Base_p(*self.base_locations['base'+str(i+1)]) for i in range(4)]

    def players_run(self):
         for i in range(3,-1,-1):
            if self.bases[i].player != None and self.bases[i].player.check_base:
